...
async def get_google_sso():
    sso = GoogleSSO(settings.JWT_CLIENT_ID, settings.JWT_CLIENT_SECRET, settings.JWT_REDIRECT_URL)
    async with sso:
        yield sso
...
async def refresh_tokens(db: DBsession, refresh_token: RefreshToken):
    ...
    session.expiration_time = datetime.now(timezone.utc) + settings.JWT_REFRESH_TOKEN_TIME
